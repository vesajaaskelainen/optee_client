		     ...
		     CK_ULONG length)
{
	...
	in_shm = ckteec_alloc_shm(length, CKTEEC_SHM_IN);
	if (!in_shm) {
		rv = CKR_HOST_MEMORY;
		goto out;
	}

	memcpy(in_shm->buffer, seed, length);
			 ...
			 CK_ULONG length)
{
	...
	out_shm = ckteec_alloc_shm(length, CKTEEC_SHM_OUT);
	if (!out_shm) {
		rv = CKR_HOST_MEMORY;
		goto out;
	}
	...
	memcpy(data, out_shm->buffer, out_shm->size);
